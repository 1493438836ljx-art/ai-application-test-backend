 ...
 */
@Data
@Schema(description = "节点类型创建请求")
public class NodeTypeCreateRequest {

    @NotBlank(message = "节点类型编码不能为空")
    @Size(max = 50, message = "节点类型编码长度不能超过50")
    @Schema(description = "节点类型��码", example = "textClean", required = true)
    private String code;

    @NotBlank(message = "节点类型名称不能为空")
    @Size(max = 100, message = "节点类型名称长度不能超过100")
    @Schema(description = "节点类型名称", example = "文本清洗", required = true)
    private String name;
